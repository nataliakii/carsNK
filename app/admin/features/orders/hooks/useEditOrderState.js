...
export function useEditOrderState({
  ...
  useEffect(() => {
    ...
    const rentalEndDateAthens = fromServerUTC(order.rentalEndDate, tzOf(order));
    ...
    const adjustedOrder = {
      ...
      deliveryInOverride:
        order.deliveryInOverride !== undefined
          ? order.deliveryInOverride
          : null,
      deliveryOutOverride:
        order.deliveryOutOverride !== undefined
    ...
    setManualDeliveryOut(
      dout != null && dout !== undefined ? String(dout) : ""
    );
    ...
    const fetchBreakdown = async () => {
      try {
        const res = await fetch(
          `/api/admin/orders/${order._id}/price-breakdown`
        );
        if (res.ok && !cancelled) {
        ...
        if (!cancelled) {
          console.error(
            "[useEditOrderState] Error fetching stored breakdown:",
            err
          );
        }
      } finally {
  ...
  useEffect(() => {
    ...
    const shouldCalculateOnFirstOpen =
      isFirstOpen.current &&
    ...
    const isBackgroundOnly =
      skipInitialApply && Boolean(editedOrder?.totalPrice);
    ...
    const fetchTotalPrice = async () => {
      try {
        ...
        const timeOutAthens = endTime
          ...
          : null;
        const timeInServer = timeInAthens
          ? toServerUTC(timeInAthens)
          : undefined;
        ...
        if (process.env.NODE_ENV === "development") {
          console.log(
            `[useEditOrderState] Price calc request (requestId: ${requestId}):`,
            {
        ...
        if (data.ok) {
          if (
            requestId === priceCalcRequestId.current &&
            ...
            if (process.env.NODE_ENV === "development") {
              console.log(
                `[useEditOrderState] Price calc response (requestId: ${requestId}):`,
                {
                  days: safeDays,
                  totalPrice: safeTotalPrice,
                  isBackgroundOnly,
  ...
  const updateField = useCallback(
    (field, value, options = {}) => {
      ...
      const shouldUpdateTotalPrice = isExplicitTotalPriceSource(
        options?.source
      );
      ...
      setEditedOrder((prev) => {
        ...
        if (
          ...
          if (process.env.NODE_ENV === "development") {
            const priceMode =
              prev?.OverridePrice !== null && prev?.OverridePrice !== undefined
                ? "MANUAL"
                : "AUTO";
            console.log(
              `[useEditOrderState] updateField(${field}): ${priceMode} mode - reset isFirstOpen, will recalculate if AUTO`
            );
          }
        }
  ...
  const updateStartDate = useCallback(
    (dateStr) => {
      ...
      if (!isInternalOrder && newStartDate.isBefore(todayAthens, "day")) {
  ...
  const updateEndDate = useCallback(
    (dateStr) => {
      ...
      if (permissions.isCurrentOrder && !isInternalOrder) {
        const todayAthens = athensStartOfDay(formatDateYYYYMMDD(athensNow()));
    ...
    [permissions.viewOnly, permissions.isCurrentOrder, isInternalOrder]
  );
  ...
  const handleSave = useCallback(
    async (saveOptions = {}) => {
      ...
      try {
        ...
        if (
          isInternalBooking(o) ||
          (payload.offline && !isPlatformBooking(o))
        ) {
        ...
        if (
          fieldPermissions.rentalEndDate &&
          ...
          if (response.updatedOrder) {
            ...
            const updatedOrder = {
              ...
              rentalStartDate: athensStartOfDay(
                formatDateYYYYMMDD(
                  fromServerUTC(response.updatedOrder.rentalStartDate)
                )
              ),
              rentalEndDate: athensStartOfDay(
                formatDateYYYYMMDD(
                  fromServerUTC(response.updatedOrder.rentalEndDate)
                )
              ),
              ...
              OverridePrice:
                response.updatedOrder.OverridePrice !== undefined
            ...
            setManualDeliveryIn(
              sr.deliveryInOverride != null &&
                sr.deliveryInOverride !== undefined
                ? String(sr.deliveryInOverride)
                : ""
            );
            setManualDeliveryOut(
              sr.deliveryOutOverride != null &&
                sr.deliveryOutOverride !== undefined
              ...
              console.log("[useEditOrderState] Save response:", {
                ...
                customerFieldsMatch: {
                  ...
                  Whatsapp:
                    ...
                    (payload.Whatsapp ?? order.Whatsapp),
                  Telegram:
                    updatedOrder.Telegram ===
                    (payload.Telegram ?? order.Telegram),
                  flightNumber:
                    updatedOrder.flightNumber ===
                    (payload.flightNumber ?? order.flightNumber),
                },
              });
            }
          }
  ...
  }, [permissions, editedOrder, setCarOrders, fetchAndUpdateOrders, onClose]);
  ...
  const lockedSavedBreakdown = useMemo(
    () => storedBreakdown,
    [storedBreakdown]
  );
