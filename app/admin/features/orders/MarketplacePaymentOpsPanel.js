...
export default function MarketplacePaymentOpsPanel({
  order,
  isSuperAdmin,
  currentUser,
  onOrderUpdated,
  ...
  const actorCompanyId =
    currentUser?.viewAsCompanyId || currentUser?.ownerId || "";
  const isOwningAdmin =
    !isSuperAdmin && idsMatch(actorCompanyId, order?.ownerId);
  if (!isSuperAdmin && !isOwningAdmin) return null;
  return (
    <MarketplacePaymentOpsBody
      order={order}
...
function MarketplacePaymentOpsBody({
  order,
  isSuperAdmin,
  currentUser,
  onOrderUpdated,
  ...
  const isOwningAdmin =
    !isSuperAdmin && idsMatch(actorCompanyId, order?.ownerId);
  const canUsePanel = isSuperAdmin || isOwningAdmin;

  const load = useCallback(async () => {
    if (!orderId || !canUsePanel) return;
    setLoading(true);
    setError("");
    try {
  ...
  const act = async (action) => {
    ...
    if (
      ...
      !window.confirm(
        "Create a new payment link and email it to the customer? The amount, currency and booking dates stay unchanged."
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (json.order && typeof onOrderUpdated === "function") {
        onOrderUpdated(json.order);
      }
      if (action === "reissue") {
        setNotice("New payment link sent to the customer.");
      } else {
        await load();
      }
    } catch (err) {
  ...
  const canResend = Boolean(isSuperAdmin && snapshot?.canResendExisting && !canIssue);
  ...
  if (!canUsePanel || !marketplace || !orderId) return null;
        ...
        <Box sx={{ mt: 1 }}>
          ...
          <Box sx={{ mb: 0.75 }}>
            <BookingPaymentStatusChip
              order={snapshot.order || order}
              chip={snapshot.paymentStatusChip}
            />
          </Box>
          <Row
          ...
          <Row
            ...
            value={formatMarketplaceEuro(
              snapshot.price?.platformAmountMinor ??
                snapshot.price?.prepaymentMinor
            )}
          />
          <Row
            label={splitLabels.collectFromCustomer}
            value={formatMarketplaceEuro(
              snapshot.price?.supplierBalanceMinor ??
                snapshot.price?.balanceMinor
            )}
          />
          <Typography
            variant="caption"
          ...
          >
            ...
            <AccordionDetails>
              ...
              <Row
                label="Refund status"
                value={snapshot.payment?.refundStatus}
              />
              <Row
                label="Dispute status"
                value={snapshot.payment?.disputeStatus}
              />
              <Row
                label="Last checkout error"
                value={
                  snapshot.invalidation?.lastErrorCategory ||
                  snapshot.payment?.lastInvalidateErrorCategory
                }
              />
              <Row
              ...
              />
              {snapshot.invalidation?.pending ||
              snapshot.invalidation?.canRetry ? (
                <Box
                ...
                >
                  <Row
                    label="Invalidation"
                    value={
                      snapshot.invalidation.pending ? "pending retry" : "idle"
                    }
                  />
                  <Row
                    label="Session type"
          ...
          >
            <Button
              ...
              onClick={() => act("resend")}
              sx={{ display: isSuperAdmin ? "inline-flex" : "none" }}
            >
            ...
            <Button
              ...
              onClick={() => act("reissue")}
            >
              {busy === "reissue" ? "Creating…" : "Create replacement link"}
            </Button>
          </Stack>
          {isSuperAdmin && reason === PAYMENT_LINK_REISSUE_REASONS.OTHER ? (
            <TextField
              size="small"
              fullWidth
              sx={{ mt: 1 }}
          ...
          {isSuperAdmin && complianceOverride ? (
            <TextField
              size="small"
              required
              fullWidth
            ...
            >
              ...
              {(() => {
                ...
                const previous = Number(
                  order?.authoritativePrice?.grossMinor || 0
                );
                const paidMinor = paidPlatformAmountMinor(order);
                return (
                  <>
                    <Typography variant="body2">
                      Previous total: {formatMarketplaceEuro(previous)}
                    </Typography>
                    {revisedGrossMinor != null ? (
                      <>
                        <Typography variant="body2">
                          New total: {formatMarketplaceEuro(revisedGrossMinor)}
                        </Typography>
                        <Typography variant="body2">
                          Already paid: {formatMarketplaceEuro(paidMinor)}
                        </Typography>
                        {preview.ok ? (
                          <Typography variant="body2">
                            New amount to collect:{" "}
                            {formatMarketplaceEuro(
                              preview.revisedSupplierBalanceMinor
                            )}
                          </Typography>
                      ...
                      <FormControlLabel
                        control={
                          <Checkbox
                            ...
                            onChange={(e) =>
                              setConfirmZeroCollect(e.target.checked)
                            }
                          />
                        }
                    ...
                    <Button
                      ...
                      onClick={() => act("correct_price").catch(() => {})}
                    >
                      {busy === "correct_price"
                        ? "Saving…"
