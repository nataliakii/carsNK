...
const quoteCoordinator = createQuoteCoordinator({
  fetchQuote: ({ carId, startDate, endDate, placeIn, placeOut, insurance }) =>
...
export function useCarCalendar({
  ...
  useEffect(() => {
    ...
    return () => {
      // Leave the shared flight running so a remount can join it. Discarding
      // the result is enough: `active` stops this instance painting a stale quote.
      active = false;
    };
  }, [
    enabled,
