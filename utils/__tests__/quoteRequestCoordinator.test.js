...
describe("quote request coordinator", () => {
  ...
  test("more than maxConcurrent different cars wait rather than stampeding", async () => {
    const gates = [deferred(), deferred(), deferred(), deferred(), deferred()];
    let started = 0;
    const fetchQuote = jest.fn().mockImplementation(() => {
      const gate = gates[started];
      started += 1;
      return gate.promise;
    });
    const coordinator = createQuoteCoordinator({ fetchQuote, maxConcurrent: 3 });

    const requests = ["a", "b", "c", "d", "e"].map((id) =>
      coordinator.request({ ...REQUEST, carId: `car-${id}` })
    );

    expect(fetchQuote).toHaveBeenCalledTimes(3);

    gates[0].resolve({ ok: true, totalPrice: 1 });
    await requests[0];
    expect(fetchQuote).toHaveBeenCalledTimes(4);

    gates[1].resolve({ ok: true, totalPrice: 1 });
    gates[2].resolve({ ok: true, totalPrice: 1 });
    gates[3].resolve({ ok: true, totalPrice: 1 });
    gates[4].resolve({ ok: true, totalPrice: 1 });
    await Promise.all(requests);
    expect(fetchQuote).toHaveBeenCalledTimes(5);
  });
});
