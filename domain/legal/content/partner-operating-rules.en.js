  ...
  content: {
    ...
    sections: [
      ...
      },
      {
        ...
        body: "A replacement already known before the Customer pays the Booking Fee is not a later breakdown. In that case the Supplier offers an equivalent replacement through the Platform. The Customer must be told that the originally requested vehicle is unavailable and must see the guaranteed class, the same transmission, the seats and the same total price. The Booking Fee is charged only after the Customer accepts that disclosed offer. If the Customer does not accept, no Booking Fee is charged. This pre-payment offer must not be described as an emergency replacement.\n\nA later replacement may be offered only where a confirmed vehicle genuinely cannot be supplied because of breakdown, accident, safety or another genuine operational reason. The replacement must be of the same or a higher class, must have the same transmission, must have sufficient seats and luggage capacity, and must not increase the rental price.\n\nThe Supplier must notify the Operator of the need for a later replacement at least {{settings.replacementNotificationHours}} hours before the scheduled handover, or immediately where the circumstances arise later than that.\n\nThe Customer must be shown the replacement and must give explicit consent through the Platform before the substitution takes effect. Substitution at the desk without prior recorded consent is prohibited. The later-breakdown rule must not be used to hide a replacement that was already known before payment.",
      },
      {
        id: "12",
