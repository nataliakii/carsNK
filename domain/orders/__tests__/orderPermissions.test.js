...
describe("orderPermissions RBAC", () => {
  ...
  describe("Superadmin permissions", () => {
    test("canEditOrder: superadmin can edit client and internal orders", () => {
    ...
    test("canDeleteOrder: superadmin can delete client orders, not internals", () => {
      ...
      expect(canDeleteOrder(pastConfirmedOrder, superadmin).allowed).toBe(
        false
      );
      expect(canDeleteOrder(pastPendingOrder, superadmin).allowed).toBe(false);
    });

    test("canEditOrderField: superadmin can edit internal company bookings", () => {
      const fields = [
        "customerName",
        "phone",
        "email",
      ...
      fields.forEach((field) => {
        expect(canEditOrderField(clientOrder, superadmin, field).allowed).toBe(
          true
        );
        expect(canEditOrderField(adminOrder, superadmin, field).allowed).toBe(
          true
        );
        expect(
          canEditOrderField(superadminOrder, superadmin, field).allowed
        ).toBe(true);
      });
    });

    test("canConfirmOrder: superadmin can toggle internal company bookings", () => {
  ...
  describe("Admin edit field permissions (client orders)", () => {
    ...
    test("ADMIN can edit non-PII fields on internal orders (my_order=false)", () => {
      expect(
        canEditOrderField(adminOrder, admin, "rentalStartDate").allowed
      ).toBe(true);
      expect(canEditOrderField(adminOrder, admin, "timeIn").allowed).toBe(true);
      expect(canEditOrderField(adminOrder, admin, "totalPrice").allowed).toBe(
        true
      );
    });

    test("ADMIN can edit client PII on internal FUTURE orders (policy: canEditClientPII true)", () => {
      expect(canEditOrderField(adminOrder, admin, "customerName").allowed).toBe(
        true
      );
  ...
  describe("Admin confirmation permissions", () => {
    ...
    test("SUPERADMIN can confirm internal company bookings", () => {
      expect(canConfirmOrder(clientOrder, superadmin).allowed).toBe(true);
      expect(canConfirmOrder(adminOrder, superadmin).allowed).toBe(true);
    });
  });
  ...
  describe("Edge cases", () => {
    test("non-admin user cannot perform any actions", () => {
      ...
      expect(
        canEditOrderField(clientOrder, nonAdmin, "customerName").allowed
      ).toBe(false);
