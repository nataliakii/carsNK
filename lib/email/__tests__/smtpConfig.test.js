...
import { getAdminTransferEmails, getInternalNotificationEmail, getPublicContactEmail, getSuperadminNotificationEmails, GREECE_ADMIN_CC_EMAIL, GREECE_INTERNAL_EMAIL, ROVARO_MAILBOX } from "@config/email";
...
describe("internal notification recipients", () => {
  ...
  const originalSuperadmin = process.env.AUTH_SUPERADMIN_EMAIL;
  ...
  function clearRecipientOverrides() {
    ...
    delete process.env.AUTH_SUPERADMIN_EMAIL;
  }
  ...
  afterEach(() => {
    ...
    if (originalSuperadmin === undefined) delete process.env.AUTH_SUPERADMIN_EMAIL;
    else process.env.AUTH_SUPERADMIN_EMAIL = originalSuperadmin;
  });
  ...
  test("superadmin notify includes ops inbox and login mailbox", () => {
    process.env.NEXT_PUBLIC_SITE_COUNTRY = "ES";
    clearRecipientOverrides();
    process.env.MAIL_INTERNAL_TO = "admin@rovaro.autos";
    process.env.AUTH_SUPERADMIN_EMAIL = "ops-login@example.com";
    expect(getSuperadminNotificationEmails()).toEqual([
      "admin@rovaro.autos",
      "ops-login@example.com",
    ]);
  });

  test("superadmin notify dedupes the same mailbox", () => {
    process.env.NEXT_PUBLIC_SITE_COUNTRY = "ES";
    clearRecipientOverrides();
    process.env.MAIL_INTERNAL_TO = "admin@rovaro.autos";
    process.env.AUTH_SUPERADMIN_EMAIL = "admin@rovaro.autos";
    expect(getSuperadminNotificationEmails()).toEqual(["admin@rovaro.autos"]);
  });
});
