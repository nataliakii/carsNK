...
} from "@/lib/email/emailTestingMode";
import { recordOutboundMail } from "@/domain/mail/recordOutboundMail";
 ...
 * When EMAIL_TESTING=true, never delivers to the original recipients
 * (redirect to EMAIL_TEST_ADDRESS or skip SMTP entirely).
 */
export async function sendEmailDirect({
  ...
  const intendedTo = normalizeRecipientList(to);
  if (intendedTo.length === 0) intendedTo.push(getInternalNotificationEmail());
  const intendedCc = normalizeRecipientList(cc);

  const testing = applyEmailTestingRecipients({
    to: intendedTo,
    cc: intendedCc,
  });
  const subject = withEmailTestingSubject(title, testing.subjectPrefix);
  assertNoHeaderInjection(subject, "subject");

  const toList = testing.deliver ? testing.to : [];
  const ccList = testing.deliver ? testing.cc : [];
  ...
  if (testing.subjectPrefix) {
    const priorPayload =
      baseMeta.payload && typeof baseMeta.payload === "object"
        ? baseMeta.payload
        : {};
    baseMeta.payload = {
      ...priorPayload,
      emailTesting: {
        enabled: true,
        intendedTo: testing.intendedTo,
        intendedCc: testing.intendedCc,
        redirected: testing.redirected,
        delivered: testing.deliver,
        actualTo: toList,
      },
    };
  }
  ...
  const logBase = {
    to: testing.deliver ? toList : testing.intendedTo,
    cc: testing.deliver ? ccList : testing.intendedCc,
    from: sender.fromHeader,
    replyTo: resolvedReplyTo,
    subject,
    html: finalHtml,
    text: finalText,
    attachmentNames: parsedAttachments.map((item) => item.filename).filter(Boolean),
    meta: baseMeta,
  };
  ...
  if (!testing.deliver) {
    await recordOutboundMail({
      ...
      error: "EMAIL_TESTING: SMTP skipped (no EMAIL_TEST_ADDRESS sink)",
    });
    return {
      messageId: "",
      accepted: [],
      skipped: true,
      reason: "EMAIL_TESTING",
      intendedTo: testing.intendedTo,
    };
  }
