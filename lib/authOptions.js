...
import {
  isAdminAccessTokenLive,
  resolveScopedAccessToken,
} from "@/domain/auth/scopedAccessToken";
import { loadAdminUserForAccessLink } from "@/domain/auth/adminAccessLink";
import { isAdminSignInDisabled } from "@/domain/admin/companyAdmins";
...
function toSessionUser({
  ...
  return {
    ...
    viaAccessLink: Boolean(viaAccessLink),
...
async function resolveCompanyName(ownerId) {
  ...
  try {
    await connectToDB();
    const doc = await Company.findById(id).select("name").lean();
    return doc?.name ? String(doc.name).trim() : "";
  } catch {
    return "";
  }
}

async function toSessionUserWithCompany(args) {
  const user = toSessionUser(args);
  if (Number(user.role) !== ROLE.SUPERADMIN) {
    user.companyName = await resolveCompanyName(user.ownerId);
  }
  return user;
}
...
/** Stamp lastLoginAt — backs the "Last login" column and clears "pending". */
async function recordSuccessfulLogin(dbUser) {
  ...
  try {
    ...
    console.error("❌ lastLoginAt update failed:", error?.message || error);
  }
}
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        if (loginToken) {
          ...
          if (!access) {
            console.log("❌ Invalid or expired company login link");
            return null;
          }
          const loaded = await loadAdminUserForAccessLink(access.tokenDoc);
          if (!loaded?.user) {
            console.log("❌ Company login link has no valid admin user");
            return null;
          }
          const dbUser = loaded.user;
          if (isAdminSignInDisabled(dbUser)) {
            console.log("❌ Access disabled for admin:", dbUser.email);
            return null;
          }
          await recordSuccessfulLogin(dbUser);
          return toSessionUserWithCompany({
            id: dbUser._id,
            ...
            companyId: COMPANY_ID,
            viaAccessLink: true,
            accessExpiresAt: access.tokenDoc.expiresAt
              ? new Date(access.tokenDoc.expiresAt).toISOString()
              : null,
            accessTokenHash: access.tokenDoc.tokenHash,
          });
        }
        ...
        if (isLocalDevAuthBypassEnabled() && !email && !password) {
          try {
            ...
            if (dbSuper) {
              console.log("✅ Local empty credentials → DB Superadmin");
              return toSessionUserWithCompany({
                id: dbSuper._id,
          ...
          if (envSuper) {
            console.log("✅ Local empty credentials → env Superadmin");
            return toSessionUserWithCompany(envSuper);
          }

          // Dev-only synthetic superadmin when DB/env users are missing
          console.log("✅ Local empty credentials → synthetic Superadmin");
          return toSessionUserWithCompany({
        ...
        try {
          ...
          if (dbUser?.password) {
            ...
            if (isAdminSignInDisabled(dbUser)) {
              console.log("❌ Access disabled for admin:", email);
              return null;
            }
            await recordSuccessfulLogin(dbUser);
            return toSessionUserWithCompany({
        ...
        try {
          ...
          return toSessionUserWithCompany(envUser);
        } catch (error) {
  ...
  callbacks: {
    jwt: async ({ token, user }) => {
      if (user) {
        ...
        token.viaAccessLink = Boolean(user.viaAccessLink);
        token.accessExpiresAt = user.accessExpiresAt || null;
        token.accessTokenHash = user.accessTokenHash || null;
        token.invalidAccessLink = false;
      }
      if (token.viaAccessLink) {
        const expired =
          token.accessExpiresAt &&
          Date.now() >= new Date(token.accessExpiresAt).getTime();
        const live = token.accessTokenHash
          ? await isAdminAccessTokenLive(token.accessTokenHash)
          : false;
        if (expired || !live) {
          token.invalidAccessLink = true;
          token.isAdmin = false;
        }
      }
      return token;
    ...
    session: async ({ session, token }) => {
      if (session.user) {
        ...
        session.user.viaAccessLink = Boolean(token.viaAccessLink);
        session.user.accessExpiresAt = token.accessExpiresAt || null;
        session.user.invalidAccessLink = Boolean(token.invalidAccessLink);
      }
      return session;
    },
